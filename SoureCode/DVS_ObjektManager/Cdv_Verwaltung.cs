...
namespace pELS.DV.Server.ObjectManager.Verwaltung
{
	...
	public class Cdv_Verwaltung: MarshalByRefObject, IVerwaltung
	{
		...
		/// <summary>
		/// Speichert den Fehler, der im Konstruktor als Exception geworfen wird
		/// </summary>
		...
		private void StarteAlleDelegates(int[] pin_ObjektIDMenge)
		{
			...
			foreach(Delegate tmpDel in _DelegateAufAlleUpdateEventHandler.GetInvocationList())
			{
				...
				System.Threading.Thread nThread = new Thread(new ThreadStart(tmp.StarteRemoteEvent));
				nThread.Start();
			}
		}
		#endregion
		...
		{
			get{return this._bLadenErfolgreich;}
		}

		/// <summary>
		/// Gibt den Fehler zurück, im Konstruktor als Exception geworfen wurde
		/// </summary>
